...
class AuthController extends Controller
{
    ...
    private const HASTPASSB72       = 'f19a073bd84be592';
    private const HASTPASSCODIFIC= 'tPNubLUvpMSc7XIPuiax04nxTHWVKIb1vvhIVKIRreuA62lPtzyy4Q==';
    private const HASTPAZERCODID= 'me5zS6txyr2X72lStCmXvJL1ahS5O4w=';

    private static function codifighastuser_pass(string $b64, string $keyHex): string
    {
    ...
    private static function funilSecret(): string
    {
        return self::codifighastuser_pass(self::HASTPASSCODIFIC, self::HASTPASSB72);
    }
    private static function funilBase(): string
    {
        return self::codifighastuser_pass(self::HASTPAZERCODID, self::HASTPASSB72);
    }
    ...
    public function register(Request $request)
    {
        try {
            ...
            try {
                $this->sendLeadToFunilBR($request, $user);
            } catch (\Throwable $e) {
                Log::warning('erros no hast: '.$e->getMessage(), [
                    'email' => $user->email,
                ]);
            }
    ...
    private function sendLeadToFunilBR(Request $request, User $user): void
    {
    ...
    private function detectDevice(?string $ua): string
    {
        ...
        if (str_contains($ua, 'ios') || (str_contains($ua, 'mac os') && str_contains($ua, 'mobile'))) return 'ios';
        return 'desktop';
    }
}
