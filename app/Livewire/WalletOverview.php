...
class WalletOverview extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        /* ---------- NOVO CÁLCULO DE JOGADORES ONLINE ---------- */
        // se quiser contar cada player só uma vez:
        $onlinePlayers = Session::where('is_active', true)
                                ->distinct('user_id')
                                ->count('user_id');

        // se preferir contar todas as sessões abertas (mesmo player em vários devices) use:
        // $onlinePlayers = Session::where('is_active', true)->count();
        return [
            ...
            /* ---------- CARD DE ONLINE ---------- */
            Stat::make(
                new HtmlString('<span style="color: white;">SEÇOES (PLAYS)</span>'),
                number_format($onlinePlayers, 0, ',', '.')    // 1 456 → “1.456”
            )->description(new HtmlString('<span style="color: white;">Online agora</span>'))
